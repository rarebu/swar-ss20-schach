...
RUN cd /swar-ss20-schach && git pull && git checkout -t origin/slick && /etc/mysql/debian.cnf && PASSWORD=$(cat /etc/mysql/debian.cnf | grep "password" | sed -n -e 's/^.*= //p') sbt compile
