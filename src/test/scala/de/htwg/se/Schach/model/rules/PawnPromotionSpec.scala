...
class PawnPromotionSpec extends WordSpec with Matchers {
  "A Pawn Promotion Object" when {
    "new" should {
      ...
      val pawn = Pawn(Colour.black, false)
      val input = "♝"
      val matrix = new Matrix[Cell]((row, col) => {
        val a = (row, col) match {
          case (6, 0) => Option.apply(Pawn(Colour.black, ability = false))
          case _ => Option.empty
        }
        if (row % 2 == 0)
      ...
      "find figure" in {
        PawnPromotion.findFigure(Colour.black, input) should be(Option.apply(Bishop(Colour.black)))
      }
      "change pawn" in {
